        ...
        <div class="row items-center">
          <div class="col-xxl-4 col-xl-4 col-lg-4 col-md-3 col-sm-6 col-xs-12">
            <q-breadcrumbs class="text-brown text-weight-bold text-h3">
              ...
              <q-breadcrumbs-el icon="o_calendar_view_month" label="Monthly Target Plan" />
            </q-breadcrumbs>
          </div>
            ...
            <div class="row items-center justify-end no-wrap">
              ...
              <div class="q-ml-sm">
                <q-btn
                  ...
                  @click="onAddMonthlyPlan(activeProjectPlanId, activeProjectId)"
                />
                <q-btn
                  v-if="showBackButton"
                  icon="o_chevron_left"
                ...
                <q-tr :props="props" :class="activeProjectPlanId == props.row.id ? 'highlight' : ''">
                  ...
                  <q-td class="text-center actions">
                    <q-btn
                      ...
                      size="1em"
                      icon="o_north_east"
                      class="cursor-pointer no-padding q-mr-xs remove-q-btn-box-shadow"
                    ...
                    <q-btn dense flat icon="o_more_vert" color="primary">
                      ...
                      <q-menu auto-close>
                        <q-list style="min-width: 180px">
                          ...
                          <q-item v-ripple clickable @click="$router.push({ path: '/project-targetplan/weeklyplanner', state: {projectId: props.row.projectId } })">
                          ...
                          <q-item v-if="isChatAccess" v-ripple clickable @click="onAddNotesToProject(props.row.projectId, 'Project Monthly Plan', props.row.projectId, props.row.name, props.row.name)">
              ...
              <table class="WhiteTable full-width">
                <thead style="position: sticky; top: 55px; z-index: 9;">
                  ...
                  <tr class="text-center">
                    <th style="min-width: 75px;">Month</th>
                    <th width="42%">Planned</th>
                    <th width="42%">Actual</th>
                ...
                <tbody
                  v-for="planDate in projectMonthlyPlanDates"
                  v-else
                  :key="planDate.id"
                  class="MonthlyDate text-grey-9"
                  @mouseenter="!planDate.isApproved ? projectMonthlyPlanDateLineHovered = planDate.id : null"
                  @mouseleave="!planDate.isApproved ? projectMonthlyPlanDateLineHovered = null : null"
                >
                  <tr
                  ...
                  >
                    ...
                    >
                      ...
                      <div class="planDateActions">
                        <q-btn
                          ...
                          :disable="!(projectMonthlyPlanDateLineHovered === planDate.id)"
                          icon="o_add"
                          color="grey-7"
                          size="sm"
                          class="q-mr-xs q-mb-xs"
                          @click="addMonthlyPlanDateLines(planDate)"
                        >
                        ...
                        >
                          <q-tooltip>
                            Click Here To Mark As Completed
                          </q-tooltip>
                        </q-btn>
                        <q-btn
                            ...
                            <div class="TaskActivity flex items-center">
                              ...
                              <div class="q-mr-xs">
                                ...
                                <q-popup-edit
                                  ...
                                  style="width: 280px;"
                                  @before-show="() => { resourceModel.value = { id: uid(), employeeId: null, estimatedHours: '' }; addResourceBtn = true; weeklyEmployeeDropdownSingleSelect.load(activeProjectId, planTypeId, planDate.weekDate)}"
                                >
                              ...
                              v-html="
                                ...
                                !line.expectedDescription ||
                                  ...
                                  line.expectedDescription.trim().toLowerCase() === '<br>' ?
                                    ...
                                    (!planDate.isApproved ? 'Double Click To Enter Your Monthly Plan' : 'No Description Available') +'</span>'
                                  : line.expectedDescription || ''"
                            />
                            <q-icon
                    ...
                    <td :class="isFullAccess && !planDate.isCompleted ? 'cursor-pointer' : ''" @dblclick="isFullAccess && handleActualEdit(isFullAccess, line, planDate.isCompleted)">
                      <template v-if="activeActual.rowId === line.id">
                        <q-editor
                          v-model="line.actualDescription"
                          class="full-width cursor-auto"
                          :toolbar="toolbar"
                          :fonts="fonts"
                        />
                      ...
                      <template v-if="line.actualDescriptionUpdatedOnUtc">
                        <div class="LastUpdatedBy text-capitalize">
                    ...
                    <td class="text-center">
                      <q-btn
                        ...
                        @click="deleteMonthlyPlanDatesLine(planDate.projectWeeklyPlanDatesLines, line, lineIndex)"
                      >
                    ...
                    <td colspan="3">
                      <div class="row">
                        <div v-if="filterRequestMapping(planDate.projectWeeklyPlanDatesReqTaskIssueMapping, 'Requirement')?.length > 0" class="col-4 flex">
                          <h3 class="q-mr-sm">Requirement</h3>
                          <q-badge
                        ...
                        <div v-if="filterRequestMapping(planDate.projectWeeklyPlanDatesReqTaskIssueMapping, 'Tasks')?.length > 0" class="col-4 flex">
                          ...
                          >
                            ...
                            <q-icon name="o_info" class="q-ml-xs"><q-tooltip>{{ item.task.name }}</q-tooltip></q-icon>
                          </q-badge>
                        </div>
                        <div v-if="filterRequestMapping(planDate.projectWeeklyPlanDatesReqTaskIssueMapping, 'Issue')?.length > 0" class="col-4 flex">
                          ...
                          <q-badge
                            ...
                            :key="item.id"
                            class="q-mr-sm cursor-pointer"
                            @click="onIssueView(item.issueId)"
                          >
                            {{ item.issue.issueNumber }}
                            <q-icon name="o_info" class="q-ml-xs"><q-tooltip>{{ item.issue.name }}</q-tooltip></q-icon>
                          </q-badge>
                        </div>
                      </div>
                    </td>
                ...
                <tbody v-if="projectMonthlyPlanDates?.length === 0">
                  <tr>
                    <td colspan="4">
  ...
  <q-dialog v-model="showPlanApproverDialog" persistent>
    <q-card style="min-width: 300px;">
      ...
      <q-card-actions align="right">
        ...
        <q-btn flat label="Add" color="primary" :disable="!selectedPlanApproverId" @click="addProjectPlanApprover(), showPlanApproverDialog = false" />
      </q-card-actions>
    </q-card>
  </q-dialog>
...
import addMonthlyPlanner from "modules/project-targetplan/components/_addMonthlyPlanner.vue";
...
const projectMonthlyPlanDateLineHovered = ref(null);
...
const {
  ...
  storageKey: "monthly-Planner-Index",
  siteId,
...
const LoadProjectPlan = async (weeklyPlan) => {
  try {
    ...
    projectMonthlyPlanDates.value = await projectService.getWeeklyProjectPlanInDetail(weeklyPlan.projectId, planTypeId.value, 0, takeCount.value);
...
// --------------------------------------------------------------------------------------------------------------------------------------------------
// Add Plan: Open pop-up on Add click
// --------------------------------------------------------------------------------------------------------------------------------------------------

// View Project Popup
const onAddMonthlyPlan = (id, projectId) => {
  activeProjectPlanId.value = id;
  $q.dialog({
    component: addMonthlyPlanner,
...
const approveThisWeeklyPlan = async (planDateId, isLock) => {
  if (planDateId) {
    $q.dialog({
      ...
      html: true,
      message: `
        <span class='fs-15'>Are you sure you want to ${isLock ? "lock" : "unlock"} this monthly Plan?</span><br>
...
const addPlanCompletionPercentage = async (planDateId, completionPercentage) => {
  if (planDateId && completionPercentage > 0) {
    ...
    if (resp) {
      ...
      notifySuccess({ message: "Monthly Plan Completed" });
    } else {
...
const calendarRef = ref(null); // the q-date component
...
const addMonthlyPlanDate = async (monthDate) => {
  if (isWeekendAlreadyPlanned(monthDate, projectMonthlyPlanDates.value)) {
    notifyError({ message: "Duplicate Monthly" });
  } else {
    const newId = uid();
    const model = ref({
      id: newId,
    ...
    projectMonthlyPlanDates.value.unshift(resp);
    // Get All Projects
    const props = { pagination: pagination.value };
    getProjects(props);
  }
...
function isWeekendAlreadyPlanned (monthDate, list) {
  const formattedMonthDate = date.formatDate(monthDate, "YYYY-MM-DD");
  return list.some(item => date.formatDate(item.weekDate, "YYYY-MM-DD") === formattedMonthDate);
}
...
const handleActualEdit = (isFullAccess, line, isCompleted) => {
  if (isFullAccess && !isCompleted) {
    activeActual.value = { rowId: line.id };
    lastDescriptionBeforeClose.value = line.actualDescription;
  }
};

const addMonthlyPlanDateLines = async (planDates) => {
  ...
  if (!hasEmptyDescriptions) {
    ...
    activeExpected.value.rowId = resp.id;
    resp.expectedDescription = resp.expectedDescription || "";
  } else {
...
const saveExpectedPlan = async (line) => {
  ...
  try {
    ...
    console.error("Error while saving project to monthly plan:", error);
  } finally {
    setTimeout(() => {
...
const saveActualLine = async (line) => {
  actualLineProcessing.value = true;
  try {
    ...
    setTimeout(() => {
      actualLineProcessing.value = false;
    }, 1500);
  }
};

const deleteMonthlyPlanDatesLine = (weeklyDatesLines, line, lineIndex) => {
  $q.dialog({
    title: "Delete Plan?",
    message: "Are you sure you want to delete monthly plan?",
    cancel: true,
    persistent: true
  }).onOk(() => {
...
const onAddResourceToLine = async (weekPlan, line, resource) => {
  ...
  try {
    ...
    getResourceSummaryForWeekPlanById(weekPlan);
  } catch (error) {
...
const onDeleteResourceToLine = (weekPlan, line, resource, resourceIndex) => {
  $q.dialog({
...
const { activeEmployeesDropdown, activeEmployeesDropdownSingleSelect } = employeeModule();
