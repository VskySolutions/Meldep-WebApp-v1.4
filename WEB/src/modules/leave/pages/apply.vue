        ...
        <div class="row items-center">
          <div class="col-12 col-md-3">
            <q-breadcrumbs class="text-brown text-weight-bold text-h3">
              ...
              <q-breadcrumbs-el label="Self Applied Leave" />
            </q-breadcrumbs>
          </div>
            ...
            <div class="row items-center justify-end no-wrap">
              ...
              <div class="q-ml-xs">
                <q-btn
                  ...
                  @click="onApplyLeaveAdd(refreshEmployeeLeaveList)"
                />
              </div>
            </div>
          </div>
      ...
      >
        ...
        <template #body="props">
          <q-tr :props="props" :class="activeRowId == props.row.id ? 'highlight' : ''">
            <q-td v-if="role === 'admin'" style="width: 12%;">
              {{ props.row.employee.person.firstName + " "+ props.row.employee.person.lastName }}
            </q-td>
                ...
                <template v-else>
                  ...
                  <span>
                    <q-icon
                      ...
                      color="primary"
                      @click="onLeaveView(props.row.id, 'applyLeave', refreshEmployeeLeaveList)"
                    />
                    <q-tooltip>
            ...
            <q-td class="text-center" style="width: 12%;">
            ...
            <q-td style="width: 16%;">
              <p v-html="stripHTML(truncate(props.row.reason, 50))" />
            </q-td>
            <q-td auto-width class="text-center actions" style="width: 10%;">
              <q-icon name="o_visibility" class="cursor-pointer q-mr-sm" @click="onLeaveView(props.row.id, 'applyLeave', refreshEmployeeLeaveList)">
                <q-tooltip>View</q-tooltip>
              </q-icon>
              <q-icon v-if="props.row.leaveStatuses.dropDownValue == 'Applied'" name="o_delete_outline" class="cursor-pointer" color="negative" @click="onDelete(props.row)">
...
const refreshEmployeeLeaveList = () => {
...
const columns = ref([
  ...
  { name: "leaveCategories.dropDownValue", label: "Leave Type", field: "leaveCategories.dropDownValue", align: "center", sortable: true },
  { name: "createdOnUtc", label: "Applied On", field: "createdOnUtc", align: "center", sortable: true },
  { name: "leaveStatuses.dropDownValue", label: "Leave Status", field: "leaveStatuses.dropDownValue", align: "center", sortable: true },
  { name: "reason", label: "Reason", field: "reason", align: "left", sortable: true }
]);

if (role === "admin") {
  columns.value.unshift({ name: "employee.person.firstName", label: "Employee Name", field: "employee.person.firstName", align: "left", sortable: true });
}

// ----------------------------------------------------------------------------------------------------------------
// DataTable:- Get All Employee leaves
// ----------------------------------------------------------------------------------------------------------------
...
const getEmployeeLeaves = (props) => {
  ...
  //   .includes(m) ? m : getCurrentMonth())(search.value.leaveMonthStr);
  search.value.leaveMonthStr = (m => Array.from({ length: 12 }, (_, i) => new Date(0, i).toLocaleString("default", { month: "long" }))
...
const onDelete = (item) => {
  ...
  zwConfirmDelete({ data: `${item.employee.person.firstName}` + " " + `${item.employee.person.lastName}` + " (From Date:" + `${item.fromDate}` + ", " + "To Date:" + `${item.toDate}` + ")" }, () => {
    leaveService.deleteEmployeeLeave(item.id).then(resp => {
      notifySuccess({ message: "Employee leave is deleted successfully." });
      refreshEmployeeLeaveList();
    });
  }, () => {
