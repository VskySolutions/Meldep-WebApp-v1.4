      ...
      <div class="q-pa-md cardTable">
        <div class="q-gutter-y-md">
          <fieldset v-if="!props.isAssign">
            <legend>Infra Database Info</legend>
            ...
            <div class="row q-col-gutter-x-md q-mb-md">
              <div class="col-12 col-md-6">
                <div class="q-mb-xs">Server Name</div>
                <div class="text-black">
                  {{ model.serverName }}
                </div>
              </div>
              <div class="col-12 col-md-6">
                <div class="q-mb-xs">Access Type</div>
                <div class="text-black">
                  {{ model.isReadOrWrite ? 'Read' : 'Write' }}
                </div>
              </div>
            </div>
            <div class="row q-col-gutter-x-md q-mb-md">
...
const getInfraDatabaseInDetailById = async () => {
  try {
    loading.value = true;
    const resp = await infraDatabaseService.getInfraDatabaseInDetailById(props.id);
    model.value = _.cloneDeep(resp);

    assignedProjectInstances.value =
      resp.infraDatabaseProjectInstanceMapping?.map(p => ({
        text: p.infraProjectInstance.platform.dropDownValue + " - " + p.infraProjectInstance.url,
        value: p.infraProjectInstance.id,
        id: p.id
      })) || [];
  } catch (error) {
    console.error("Failed to load Infra Database:", error);
  } finally {
    loading.value = false;
  }
};
...
async function assignProjectInstance (projectInstanceId) {
  ...
  const response = await infraDatabaseService.infraDatabaseAssignToProjectInstance(props.id, projectInstanceId);
...
async function removeProjectInstance (projectInstance) {
  await infraDatabaseService.deleteAssignProjectInstance(projectInstance.id);
