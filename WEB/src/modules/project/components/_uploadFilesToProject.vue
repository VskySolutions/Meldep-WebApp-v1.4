  ...
  <q-dialog ref="dialogRef" class="customDialog" persistent position="right" @hide="onDialogHide">
    <q-card class="q-dialog-plugin PersonMain card-header with-tools headerBasic" style="width: 1000px; height: 100% !important;max-width: 100vw;">
      <q-card-section class="card-header with-tools bg-primary stickyHeader">
        <div class="text-h2 text-white">{{ projectName }} Files</div>
              ...
              >
                ...
                <template #body="props">
                  <q-tr :props="props" :class="activeRowId == props.row.id ? 'highlight' : ''" :set="(preSourceName = null,preSubModuleName = null)">
                    <q-td><span v-if="preSourceName !== props.row.type" :set="preSourceName = props.row.type" class="common-q-td">{{ props.row.type }}</span></q-td>
                    <q-td><span v-if="preSubModuleName !== props.row.sub_Module" :set="preSubModuleName = props.row.sub_Module" class="common-q-td">{{ props.row.sub_Module }}</span></q-td>
                    <!-- <q-td style="width: 10%">
                    <a :href="baseURL + props.row.virtualPath" target="_blank">
                      {{ extractFileName(props.row.virtualPath) }}
                    </a>
                  </q-td> -->
                    ...
                    <q-td class="text-center actions">
                      <!-- <q-icon name="o_download" class="cursor-pointer q-mr-sm" size="xs" @click="onDownload(props.row.virtualPath)">
                      <q-tooltip>Download</q-tooltip>
                    </q-icon>
                    <a :href="baseURL + props.row.virtualPath" download target="_blank" class="q-mr-sm" rel="noopener noreferrer"><q-icon name="o_visibility" color="black" size="xs" /><q-tooltip>View</q-tooltip></a> -->
...
//   // Use Google Docs Viewer only for supported formats
//   const googleDocsViewer = supportedFormats.includes(fileExtension)
//     ? `https://docs.google.com/gview?url=${encodeURIComponent(fileUrl)}&embedded=true`
//     : fileUrl;
//   console.log("googleDocsViewer", googleDocsViewer);
...
//   // Delay to avoid null reference errors
//   setTimeout(() => {
...
//           .top-right { position: absolute; top: 15px; right: 15px; z-index: 10; }
//           iframe { width: 100vw; height: 100vh; border: none; }
//         </style>
//       </head>
//       <body>
//         <a class="top-right" href="${fileUrl}" download>
//           <button>Download</button>
//         </a>
//         <iframe src="${googleDocsViewer}" onerror="this.src='${fileUrl}'"></iframe>
//       </body>
//       </html>
//     `);
//   }, 100);
...
//       link.href = url;
//       link.setAttribute("download", file.split("/").pop()); // Ensures the file is downloaded
...
//     fileUrl = new URL(file.file.virtualPath, baseURL).href; // For uploaded files
...
const onDelete = (item) => {
  ...
  zwConfirmDelete({ data: `${item.virtualPath.split("/").pop()}` }, () => {
    projectService.deleteFile(item.id, item.type).then(resp => {
      ...
      getAllFilesByProjectId({ pagination: pagination.value });
    });
  }, () => {
...
const onSubmit = async () => {
  ...
  try {
    ...
    projectService.saveProjectFiles(formData).then((resp) => {
