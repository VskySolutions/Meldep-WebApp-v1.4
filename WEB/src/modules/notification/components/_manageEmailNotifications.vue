<template>
  <q-card class="project6">
      ...
      <div class="row items-center">
        <div class="col-12 col-xs-2 col-sm-2 col-md-2 col-lg-2 col-xl-2">
          <q-breadcrumbs class="text-brown text-weight-bold text-h3">
            ...
            <q-breadcrumbs-el label="Email Notifications" />
          </q-breadcrumbs>
        </div>
        ...
        <div class="col-12 col-xs-6 col-sm-7 col-md-7 col-lg-7 col-xl-7">
          <div class="row items-center justify-end no-wrap">
            <q-select
              ...
              map-options
              style="width:250px;"
              @update:model-value="onChangeEmployeeNotificationList"
            ...
            <q-btn
              ...
              label="Back"
              no-caps
              class="text-primary btnRounded no-space-between"
    ...
    <q-table
      ...
      :class="emailNotificationRows.length === 0 ? 'Custom-DataTable' : 'Custom-DataTable my-sticky-header-table'"
      :loading="loading"
      :rows="emailNotificationRows"
      ...
      @request="getAllSitesEmailNotificationsPermissions"
    >
      <template #loading>
      ...
      <template #body="props">
        <q-tr :props="props" :class="activeRowId == props.row.id ? 'highlight' : ''">
          <q-td width="10%">
            {{ props.row.sitesEmailNotifications.name }}
          </q-td>
          <q-td width="40%">
            {{ props.row.sitesEmailNotifications.subject }}
          </q-td>
          <q-td width="5%" class="text-center actions">
            <q-icon
              name="o_visibility"
              class="cursor-pointer "
              @click="openPreview(props.row.id)"
            >
              <q-tooltip>Email Preview</q-tooltip>
            </q-icon>
            <q-toggle
...
import manageEmailNotifications from "modules/notification/components/_emailPreview.vue";
...
const localStorageKey = "manageEmailNotifications";
...
const subject = filterLocalStorage ? filterLocalStorage.subject : "";
...
// Table variables
const tableRef = ref();
const emailNotificationRows = ref([]);
const activeRowId = ref(null);
const emailNotificationColumns = ref([
  { name: "sitesEmailNotifications.name", label: "Name", field: "sitesEmailNotifications.name", align: "left", sortable: true },
  { name: "sitesEmailNotifications.subject", label: "Subject", field: "sitesEmailNotifications.subject", align: "left", sortable: true }
]);

// Get/Map email notification list
const getAllSitesEmailNotificationsPermissions = (props) => {
  ...
  notificationsService.getAllSitesEmailNotificationsPermissions(payload).then((resp) => {
    emailNotificationRows.value = resp.sitesEmailNotificationsPermissionsList;
    const allInactive = emailNotificationRows.value.every(row => !row.active);
...
// on or off email notification permission
function onPermission (id, activeStatus) {
  setTimeout(function () {
    const status = activeStatus ? "on" : "off";
    notificationsService.updateEmailNotificationPermission(id, activeStatus, selectedUserId.value).then(resp => {
      notifySuccess({ message: `Email Notification ${status} successfully.` });
      getAllSitesEmailNotificationsPermissions({ pagination: pagination.value });
    });
  });
}

// on or off all email notification permissions
function onAllPermissions (setActive) {
  setTimeout(function () {
    const status = setActive ? "on" : "off";
    notificationsService.updateAllEmailNotificationPermissions(setActive, selectedUserId.value).then(resp => {
      notifySuccess({ message: `All Email Notifications ${status} successfully.` });
      getAllSitesEmailNotificationsPermissions({ pagination: pagination.value });
    });
  });
}
...
const openPreview = (id) => {
  ...
  $q.dialog({
    component: manageEmailNotifications,
    componentProps: { id }
  }).onOk(() => {
    getAllSitesEmailNotificationsPermissions();
  }).onCancel(() => {
  }).onDismiss(() => {
    activeRowId.value = id;
  });
...
const onSearch = () => {
  ...
  getAllSitesEmailNotificationsPermissions(propps);
};

// Clear search
const onClear = () => {
  search.value.subject = null;
...
function onChangeEmployeeNotificationList () {
  getAllSitesEmailNotificationsPermissions({ pagination: pagination.value });
}
