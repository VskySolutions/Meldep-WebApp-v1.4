...
namespace Vsky.Services.Messages
{
    public partial class SmtpBuilder : ISmtpBuilder
    {
        ...
        public SmtpBuilder(ApplicationDbContext db)
        {
            _db = db;
        }
        ...
        public virtual async Task<SmtpClient> BuildAsync(EmailAccount emailAccount = null)
        {
            ...
            {
                emailAccount = await _db.EmailAccounts.FirstOrDefaultAsync() ?? throw new Exception("Email account could not be loaded."); ;
            }

            var client = new SmtpClient { ServerCertificateValidationCallback = ValidateServerCertificate };

            try
            {
                await client.ConnectAsync(emailAccount.Host, emailAccount.Port, emailAccount.EnableSsl ? SecureSocketOptions.SslOnConnect : SecureSocketOptions.StartTlsWhenAvailable);
